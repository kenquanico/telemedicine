...
export default function AboutPage({ onBack }: { onBack?: () => void }) {
    return (
        <PageShell onBack={onBack}>
            ...
            <div className="mb-6 rounded-3xl bg-[#427b77] px-8 py-10 text-white">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
                {[
                    { icon: "🔬", title: "Clinical integrity", body: "Every order reviewed by a licensed pharmacist. We never compromise on safety." },
                    { icon: "⚡", title: "Speed without shortcuts", body: "Same-day delivery, but not at the expense of proper cold-chain and handling." },
                    { icon: "🤝", title: "Radical transparency", body: "Clear pricing, no hidden fees, and honest answers — always." },
                ].map(({ icon, title, body }) => (
                    <Card key={title}>
                        <span className="mb-3 block text-[24px]">{icon}</span>
