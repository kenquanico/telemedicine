...
const FILTER_ICON_COLORS: Record<string, string> = {
    "Open Now":      "#059669",
    "24 Hours":      "#d97706",
    "Has Parking":   "#2563eb",
    "Drive-Through": "#7c3aed",
    "Near Me":       "#e11d48",
};
...
function getMarkerIcon(pharmacy: Pharmacy): google.maps.Icon {
    ...
    const svg = `
        <svg xmlns="http://www.w3.org/2000/svg" width="${labelWidth}" height="78" viewBox="0 0 ${labelWidth} 78">
            ...
            <path d="M${x} 47C${x - 7.5} 47 ${x - 13.5} 53 ${x - 13.5} 60.5c0 9.5 13.5 17.5 13.5 17.5s13.5-8 13.5-17.5C${x + 13.5} 53 ${x + 7.5} 47 ${x} 47z" fill="${pinColor}" fill-opacity="${pinOpacity}" stroke="white" stroke-width="2"/>
            <circle cx="${x}" cy="60.5" r="4.5" fill="white" fill-opacity="0.92"/>
            <circle cx="${x}" cy="60.5" r="2.4" fill="${pinColor}" fill-opacity="${pinOpacity}"/>
        </svg>
    `;
...
function PharmacyCard({
    ...
    return (
        ...
        >
            <div
                className={`relative w-full h-[122px] rounded-2xl overflow-hidden bg-[#F0F5F4] mb-2.5 ${
            ...
            >
                {photoUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center" style={{ background: color + "22" }}>
                        <span className="text-[22px] font-black tracking-tight epilogue-header" style={{ color }}>
                            {getInitials(pharmacy.name)}
                        </span>
                    </div>
                )}
                {pharmacy.hasFreeDelivery && (
                    <span className="absolute top-2 left-2 flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-600 epilogue-header">
            ...
            <div className="px-0.5">
                <p className="text-[13px] font-bold text-[#2d2d2d] mb-1 truncate epilogue-header">{pharmacy.name}</p>
                <div className="flex items-center gap-1.5 mb-1.5">
                    {pharmacy.rating && (
                        <span className="flex items-center gap-1 text-[11px] font-semibold text-amber-700 epilogue-header">
                            <Star size={10} fill="currentColor" strokeWidth={0} />
                            {pharmacy.rating}
                            <span className="font-normal text-gray-400 text-[10px] epilogue-regular">({pharmacy.userRatingsTotal}+)</span>
                        </span>
                    )}
                    <span className="text-gray-300 text-[10px]">•</span>
                    <span
                        className={`flex items-center gap-1 text-[11px] font-semibold epilogue-header ${
                    ...
                    >
                        <span className="w-[5px] h-[5px] rounded-full bg-current opacity-85 shrink-0" />
        ...
        <div className="relative w-full overflow-hidden" style={{ height: "calc(100vh - 76px)" }}>
            ...
            <div className="absolute top-4 left-4 z-20 flex items-center gap-2.5 max-w-[calc(100vw-32px)]">
                ...
                <div className="relative flex items-center flex-shrink-0">
                    <Search size={14} strokeWidth={2} className="absolute left-3 text-[#427b77] opacity-70 pointer-events-none" />
                    <input
                        type="text"
                        value={searchValue}
                ...
                {/* Filter pills — scrollable, same row */}
                <div className="flex gap-2 overflow-x-auto scrollbar-none flex-nowrap pb-0.5">
                    {FILTERS.map((f) => {
                        const Icon      = FILTER_ICONS[f];
                        const iconColor = FILTER_ICON_COLORS[f];
                        const isActive  = activeFilters.includes(f);
                        return (
                            <button
                                key={f}
                                onClick={() => toggleFilter(f)}
                                className={`inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full border text-[12px] font-semibold cursor-pointer whitespace-nowrap transition-all shadow-[0_2px_10px_rgba(6,30,41,0.08)] epilogue-header ${
                                    isActive
            ...
            <div className="absolute bottom-0 left-0 right-0 z-20 pb-[18px] bg-gradient-to-t from-white/97 via-white/80 to-transparent pointer-events-none">
                <div className="pointer-events-auto">
                    <div className="flex items-center justify-between px-16 py-3 max-md:px-4">
                        ...
                        {!loading && (
                            <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full epilogue-header">
                    ...
                    >
                        {loading ? (
                            ...
                            <div className="flex items-center gap-2 px-5 py-4 text-gray-400 text-[13px] epilogue-regular">
            ...
            {selectedPharmacy && (
                ...
                >
                    ...
                    <div className="flex gap-3 items-start mb-3.5">
                        <div
                            className="w-[46px] h-[46px] rounded-[13px] flex items-center justify-center text-[13px] font-extrabold flex-shrink-0 epilogue-header"
                            style={{
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-[14px] font-bold text-[#2d2d2d] mb-0.5 leading-snug epilogue-header">
                                {selectedPharmacy.name}
                            </p>
                            <p className="text-[11.5px] text-gray-400 flex items-center gap-1 mb-1.5 epilogue-regular">
                                <MapPin size={10} strokeWidth={2} />
                                {selectedPharmacy.address || "See on map"}
                            </p>
                            <div className="flex gap-1.5 flex-wrap">
                                {selectedPharmacy.rating && (
                                    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10.5px] font-semibold bg-amber-50 text-amber-700 epilogue-header">
                                        <Star size={9} fill="currentColor" strokeWidth={0} />
                                        {selectedPharmacy.rating}
                                    </span>
                                )}
                                <span
                                    className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10.5px] font-semibold epilogue-header ${
                                ...
                                {selectedPharmacy.hasFreeDelivery && (
                                    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10.5px] font-semibold bg-blue-50 text-blue-600 epilogue-header">
                    ...
                    <button className="w-full bg-[#2d2d2d] text-white border-none rounded-xl py-3 px-4 text-[12.5px] font-semibold cursor-pointer flex items-center justify-center gap-1.5 hover:bg-[#427b77] transition-colors epilogue-header">
                        Reserve for Pickup
